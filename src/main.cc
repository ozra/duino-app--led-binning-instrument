...
fn readLedCurrent() -> F32 {
   let const bias_calib = 270;   //-111.0; // Still gives about -5ma error bias, but one step up gives +30mA error - when resolving here at the int-level
...
fn readPVCurrent() -> F32 {
   let const bias_calib = 260;   //-111.0; // Still gives about -5ma error bias, but one step up gives +30mA error - when resolving here at the int-level
...
/* let const COUNTER_TOP = 99; */
let const COUNTER_TOP = 25;
